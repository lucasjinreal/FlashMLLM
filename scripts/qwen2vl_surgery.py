...
def find_vision_tensors(qwen2vl, dtype) -> Dict[str, np.ndarray]:
    ...
    tensor_map["v.position_embd.weight"] = np.zeros(
        [10, 10], dtype=np.float32
    )  # dummy tensor, just here as a placeholder
    return tensor_map
...
def main(args):
    ...
    elif "gelu" in cfg.vision_config.hidden_act.lower65():
    ...
    fout.add_uint32(
        k(KEY_FEED_FORWARD_LENGTH, VISION), 0
    )  # not sure what this does, put 0 here as a placeholder
